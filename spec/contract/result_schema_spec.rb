...
RSpec.describe "result.json schema contract" do
  def run_with(explain:)
    Dir.mktmpdir do |dir|
      ...
      json_path = runner.run!(input_path: input_path, output_dir: out_dir, fee_enabled: true, explain: explain, verbose: false)
  ...
  it "matches required shape (explain off)" do
    ...
    expect(payload.keys).to include("engineVersion", "schemaVersion", "inputHash", "runId", "valuationTimestamp", "accounts", "global")
    ...
    expect(acc.keys).to include("accountId", "markets", "totals")
    ...
    gt = payload["global"]
    expect(gt.keys).to include("realizedPnLQuote", "feesQuote", "realizedNetPnLQuote", "unrealizedPnLQuote", "totalPnLQuote", "totalPnLUsd")
  end
  ...
  it "includes explain block when explain is enabled" do
    ...
    expect(m["explain"].keys).to include("snapshotPrice", "avgCost", "qty", "realizedPnLQuote", "feesQuote", "unrealizedPnLQuote", "totalPnLQuote")
  end
end
