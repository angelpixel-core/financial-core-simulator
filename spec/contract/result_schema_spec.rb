...
RSpec.describe 'result.json schema contract' do
  ...
  it 'adds replay metadata in timeline mode without breaking required shape' do
    ...
    Dir.mktmpdir do |dir|
      ...
      expect(payload).to have_key('timeline')

      timeline = payload.fetch('timeline')
      expect(timeline).to include('schema_version' => '1.0')
      expect(timeline.fetch('points')).to be_a(Array)
      expect(timeline.fetch('points').first).to include(
        'timestamp',
        'account_id',
        'market_id',
        'realized_pnl',
