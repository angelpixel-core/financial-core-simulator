...
RSpec.describe "bin/fcs run" do
  ...
  it "warns when output-dir resolves to repo root" do
    output_tmpdir do |tmp|
      ...
      expect(payload).to include(
        ...
        "what_happened" => I18n.t("fcs.cli.warnings.unsafe_output_dir.what_happened", locale: :en)
      )
  ...
  it "uses locale-specific warning text when FCS_LOCALE is set" do
    output_tmpdir do |tmp|
      ...
      payload = parse_last_json(stderr)
      expect(payload.fetch("what_happened")).to eq(
        I18n.t("fcs.cli.warnings.unsafe_output_dir.what_happened", locale: :es)
      )
    end
  end

  it "keeps warning payload keys stable across locales" do
    output_tmpdir do |tmp|
      _stdout_en, stderr_en, status_en = Open3.capture3(
        {"FCS_LOCALE" => "en"},
        ruby,
        File.join(root, "bin/fcs"),
        "run",
      ...
      _stdout_es, stderr_es, status_es = Open3.capture3(
        {"FCS_LOCALE" => "es"},
        ruby,
        File.join(root, "bin/fcs"),
        "run",
      ...
      expect(status_en.success?).to be(true)
      expect(status_es.success?).to be(true)

      payload_en = parse_last_json(stderr_en)
      payload_es = parse_last_json(stderr_es)

      expect(payload_en.keys.sort).to eq(payload_es.keys.sort)
      expect(payload_en.fetch("details").keys.sort).to eq(payload_es.fetch("details").keys.sort)
    end
  end
  ...
  it "prints details when --verbose is set" do
    output_tmpdir do |tmp|
      ...
      expect(stdout).to include(I18n.t("fcs.reporting.cli_summary.title", locale: :en))
      expect(stdout).to include("#{I18n.t("fcs.reporting.cli_summary.labels.status", locale: :en)}: success")
      expect(stdout).to include("#{I18n.t("fcs.reporting.cli_summary.sections.artifacts", locale: :en)}:")
      expect(stdout).to include(I18n.t("fcs.cli.success.ok_wrote", locale: :en, path: ""))
    end
  end

  it "prints Spanish summary labels when FCS_LOCALE is set" do
    output_tmpdir do |tmp|
      ...
      expect(status.success?).to be(true)
      expect(stdout).to include(I18n.t("fcs.reporting.cli_summary.title", locale: :es))
      expect(stdout).to include("#{I18n.t("fcs.reporting.cli_summary.labels.status", locale: :es)}: success")
      expect(stdout).to include("#{I18n.t("fcs.reporting.cli_summary.sections.artifacts", locale: :es)}:")
    end
  end
  ...
  it "prints identical CLI summary across repeated verbose runs" do
    output_tmpdir do |tmp|
      ...
      expect(stdout1).to include(I18n.t("fcs.reporting.cli_summary.title", locale: :en))
