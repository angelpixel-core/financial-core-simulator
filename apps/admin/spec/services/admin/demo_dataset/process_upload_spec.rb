...
RSpec.describe Admin::DemoDataset::ProcessUpload do
  ...
  it 'creates invalid upload when parser output is invalid' do
    ...
    allow(parser).to receive(:call).and_return(parser_result_class.new(valid?: false, input: { 'trades' => [] },
                                                                       errors: errors))

    outcome = described_class.new(parser: parser).call(file_path: '/tmp/demo.xlsx', timeline_enabled: true)
