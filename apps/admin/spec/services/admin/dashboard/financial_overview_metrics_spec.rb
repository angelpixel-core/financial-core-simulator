...
RSpec.describe Admin::Dashboard::FinancialOverviewMetrics do
  describe '#call' do
    ...
    it 'combines persisted operations from succeeded and validation-failed runs ordered by day' do
      base_run = Run.create!(status: :succeeded, input_json: { 'trades' => [] })
      failed_run = Run.create!(status: :failed, input_json: { 'trades' => [] })
      RunValidationError.create!(run: failed_run, message: 'invalid row', code: 'INVALID_SIDE')

      day_one = RunSnapshot.create!(run: base_run, operational_date: Date.new(2026, 3, 29), reporting_currency: 'USD')
      RunDailyPnl.create!(run_snapshot: day_one, realized_pnl: 2, unrealized_pnl: 3, total_pnl: 5)
      RunDailyVolume.create!(run_snapshot: day_one, notional_volume: 20, trade_count: 2, unit_type: 'quote',
                             unit_code: 'USD')

      day_two = RunSnapshot.create!(run: failed_run, operational_date: Date.new(2026, 3, 30), reporting_currency: 'USD')
      RunDailyPnl.create!(run_snapshot: day_two, realized_pnl: 1, unrealized_pnl: 1, total_pnl: 2)
      RunDailyVolume.create!(run_snapshot: day_two, notional_volume: 15, trade_count: 1, unit_type: 'quote',
                             unit_code: 'USD')

      metrics = described_class.new(run: failed_run).call

      expect(metrics[:trade_activity]).to eq([
                                               { timestamp: '2026-03-29', trade_count: 2 },
                                               { timestamp: '2026-03-30', trade_count: 1 }
                                             ])
      expect(metrics[:trade_volume]).to eq([
                                             { timestamp: '2026-03-29', volume: 20.0, unit_type: 'quote',
                                               unit_code: 'USD' },
                                             { timestamp: '2026-03-30', volume: 15.0, unit_type: 'quote',
                                               unit_code: 'USD' }
                                           ])
      expect(metrics[:pnl_daily]).to eq([
                                          { timestamp: '2026-03-29', realized_pnl: 2.0, unrealized_pnl: 3.0,
                                            total_pnl: 5.0 },
                                          { timestamp: '2026-03-30', realized_pnl: 1.0, unrealized_pnl: 1.0,
                                            total_pnl: 2.0 }
                                        ])
    end
