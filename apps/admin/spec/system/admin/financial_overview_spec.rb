  ...
  it 'renders chart containers when series are present' do
    ...
    within('[data-controller="financial-overview"]') do
      expect(page).to have_css(
        '[data-financial-overview-target="activityChart"].is-ready, [data-financial-overview-target="activityFallback"]:not([hidden])', wait: 10
      )
      expect(page).to have_css('[data-financial-overview-target="volumeChart"].is-ready', wait: 10)
      expect(page).to have_css('[data-financial-overview-target="volumeChart"] .trend-chart__shell', wait: 10)
      expect(page).to have_css('[data-financial-overview-target="pnlChart"].is-ready', wait: 10)
      expect(page).to have_css('[data-financial-overview-target="pnlChart"] .trend-chart__shell', wait: 10)
