...
module Admin
  class DashboardMetrics
    ...
    def call(trades_window: 'all-time')
      ...
      total_trades = successful_trades(selected_trades_window)
    ...
    def ingestion_validation_errors(limit: INGESTION_ERRORS_LIMIT, source: nil, field: nil)
      entries = validation_error_entries + failed_run_entries
      entries = entries.sort_by { |entry| entry[:sort_key].to_s }.reverse
    ...
    def validation_error_entries
      RunValidationError.includes(:run).order(id: :desc).map do |error|
        entry = @ingestion_validation_error_mapper.map(validation_error: error)
        entry[:sort_key] = entry[:occurred_at]
        entry
      end
    end

    def failed_run_entries
      validation_failed_runs.map do |run|
        entry = @ingestion_validation_error_mapper.map(run: run)
        entry[:sort_key] = entry[:occurred_at]
        entry
      end
    end
    ...
    def successful_trades(window_key)
      eligible_run_ids = Run.timeline_eligible.select(:id)
      scope = RunDailyVolume.joins(:run_snapshot).where(run_snapshots: { run_id: eligible_run_ids })

      days = TRADE_WINDOWS.fetch(window_key)
      if days
        start_date = (days - 1).days.ago.to_date
        scope = scope.where(run_snapshots: { operational_date: start_date..Date.current })
      end

      scope.sum(:trade_count)
    end
    ...
    def markets_for(run, payload:)
      ...
      if payload_accounts.is_a?(Array)
        from_payload = payload_accounts.filter_map do |entry|
          ...
          risk_events.filter_map do |risk_event|
