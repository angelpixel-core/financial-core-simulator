...
class Admin::DemoDatasetsController < ApplicationController
  ...
  def preview
    file = params[:file]
    if file.blank?
      render_preview(state: :error, errors: [{ code: 'MISSING_FILE' }], status: :unprocessable_entity)
      return
    end
    ...
    sample_rows = if input.is_a?(Hash)
                    Array(fetch_input_value(input, :trades)).first(
                      Admin::DemoDataset::PreviewModalComponent::SAMPLE_LIMIT
                    )
                  else
                    []
                  end
