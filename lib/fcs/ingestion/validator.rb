  ...
  module Ingestion
    class Validator
      ...
      def validate_schema_version!(h)
        sv = h["schemaVersion"]
        unless SUPPORTED_SCHEMA_VERSIONS.include?(sv)
          raise FCS::Error.new(
      ...
      def extract_unique_ids!(arr, key, code:)
        ...
        if dup
          raise FCS::Error.new(code, "Duplicate id", details: { field: key, value: dup })
        end
        ids.to_set
      end
      ...
      def validate_snapshot!(h, market_ids)
        ...
        prices = snap["prices"]
        raise FCS::Error.new(FCS::Errors::ERR_MISSING_SNAPSHOT, "Missing snapshot prices", details: {}) unless prices.is_a?(Array)
        ...
        prices.each do |p|
          ...
          validate_positive_decimal_string!(v, field: "priceSnapshot.prices.priceQuotePerBase", context: { marketId: mid })
        end
      ...
      def validate_trades!(trades, account_ids, market_ids, fee_enabled)
        trades.each do |t|
          ...
          unless %w[BUY SELL].include?(side)
            raise_invalid!("Invalid side", field: "side", details: { side: side })
          end

          validate_positive_decimal_string!(t["quantityBase"], field: "quantityBase", context: { tradeId: t["tradeId"] })
          validate_positive_decimal_string!(t["priceQuotePerBase"], field: "priceQuotePerBase", context: { tradeId: t["tradeId"] })
      ...
      def validate_decimal_string!(v, field:, context:, allow_zero:)
        if v.is_a?(Float)
          raise FCS::Error.new(FCS::Errors::ERR_INVALID_NUMBER, "Float not allowed", details: context.merge(field: field))
        end
